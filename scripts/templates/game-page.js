...
function selectionReason(deal) {
  return `This offer combines a Deal Score of ${deal.dealScore} with ${deal.userRating}% positive player feedback across ${formatCount(deal.reviewCount)} reviews.`;
}
...
function pageHead(title, description, canonical, image, schema) {
  ...
  <link rel="stylesheet" href="../style.css?v=23"><link rel="stylesheet" href="../game-pages.css?v=1">
...
function renderGamePage(deal, snapshotInput = {}) {
  ...
  const description = `${deal.title} is ${formatPrice(deal.salePrice)} at ${deal.storeName} in LootRadar's latest snapshot. See its Deal Score, player rating, and buying context.`;
  ...
  <header class="game-hero"><div class="game-art">${art}</div><div class="game-hero-copy"><p class="section-kicker">Quality-qualified PC offer</p><h1>${escapeHTML(deal.title)} PC deal and price check</h1><p class="game-lede">A current price snapshot with the review evidence and ranking context behind the offer.</p><div class="game-tags">${tags.map(tag => `<span>${escapeHTML(tag)}</span>`).join('')}</div></div></header>
  <section class="offer-panel" aria-labelledby="currentOffer"><div><p class="offer-label">Current saved offer</p><h2 id="currentOffer">${formatPrice(deal.salePrice)} at ${escapeHTML(deal.storeName)}</h2><p>${escapeHTML(deal.discount)}% below the listed normal price of ${formatPrice(deal.normalPrice)}.</p><time${snapshot.iso ? ` datetime="${snapshot.iso}"` : ''}>Prices checked ${escapeHTML(snapshot.label)}</time></div><a class="offer-button" href="https://www.cheapshark.com/redirect?dealID=${escapeHTML(safeDealID(deal.dealID))}" target="_blank" rel="sponsored noopener noreferrer" data-track-deal data-track-surface="game_price_page" data-track-store="${escapeHTML(deal.storeName)}" data-track-price="${escapeHTML(deal.salePrice)}">Check price at ${escapeHTML(deal.storeName)}</a></section>
  <section class="evidence-grid" aria-label="Deal evidence"><article><span>Deal Score</span><strong>${escapeHTML(deal.dealScore)}<small>/100</small></strong><p>LootRadar's combined ranking signal, not a game review score.</p></article><article><span>Player rating</span><strong>${escapeHTML(deal.userRating)}<small>% positive</small></strong><p>Based on ${formatCount(deal.reviewCount)} recorded player reviews.</p></article><article><span>Price cut</span><strong>${escapeHTML(deal.discount)}<small>% off</small></strong><p>${formatPrice(deal.normalPrice)} listed normally, ${formatPrice(deal.salePrice)} in this snapshot.</p></article></section>
  <section class="game-context"><div><p class="section-kicker">Why it made the cut</p><h2>The price has quality evidence behind it</h2><p>${escapeHTML(selectionReason(deal))}</p><p>LootRadar excludes obvious add-ons, bundles, Early Access listings, and thin review signals from these permanent pages.</p></div><aside><h2>What to verify before buying</h2><ul><li>Confirm the final price and edition on the retailer page.</li><li>Check platform, launcher, region, and activation requirements.</li><li>Buy for your taste and schedule, not only for the discount.</li></ul></aside></section>
  <section class="snapshot-note"><h2>About this price check</h2><p>This page reflects one saved offer from LootRadar's latest data refresh. It does not claim the price is a historical low. Retailer prices and availability can change before the next refresh.</p></section>
...
function renderGameHub(deals, snapshotInput = {}) {
  ...
  const description = 'Browse PC game price checks backed by strong player ratings, credible review volume, and LootRadar Deal Scores of 70 or better.';
  ...
  const cards = deals.map(deal => `<article class="game-card"><p>${escapeHTML(deal.storeName)} <span>${escapeHTML(deal.dealScore)} Deal Score</span></p><h2><a href="${escapeHTML(gamePageRoute(deal))}">${escapeHTML(deal.title)}</a></h2><div><strong>${formatPrice(deal.salePrice)}</strong><s>${formatPrice(deal.normalPrice)}</s><span>${escapeHTML(deal.discount)}% off</span></div><p>${escapeHTML(deal.userRating)}% positive from ${formatCount(deal.reviewCount)} reviews</p><a class="card-detail-link" href="${escapeHTML(gamePageRoute(deal))}">See price and quality details</a></article>`).join('');
  return `<!doctype html><html lang="en"><head>${pageHead(title, description, canonical, `${SITE_ORIGIN}/public/og.png`, schema)}</head><body>${header()}<main class="game-shell" id="mainContent"><nav class="game-breadcrumbs" aria-label="Breadcrumb"><a href="../index.html">Home</a><span>/</span><span aria-current="page">Game price checks</span></nav><header class="game-hub-hero"><p class="section-kicker">Permanent game pages</p><h1>PC game deals with the evidence attached</h1><p>These games clear a stricter quality bar: at least 80% positive feedback, 1,000 player reviews, and a LootRadar Deal Score of 70 or better.</p><time${snapshot.iso ? ` datetime="${snapshot.iso}"` : ''}>Prices checked ${escapeHTML(snapshot.label)}</time></header><section class="hub-rule"><strong>${deals.length} current price checks</strong><p>Every page is refreshed from the saved deal snapshot and states exactly what the data can support.</p></section><section class="game-card-grid" aria-label="Game price checks">${cards}</section><nav class="game-related" aria-label="Related pages"><a href="../deals/index.html">Browse deal collections</a><a href="../games.html">Search the full catalog</a><a href="../methodology.html">Read the scoring methodology</a><a href="../feed.xml">Follow the deal feed</a></nav></main>${footer()}</body></html>`;
}
