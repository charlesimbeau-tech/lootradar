...
function pageHead(title, description, canonical, image, schema) {
  ...
  <link rel="stylesheet" href="../style.css?v=29"><link rel="stylesheet" href="../game-pages.css?v=2">
...
function renderGamePage(deal, snapshotInput = {}, related = []) {
  ...
  const selectionReason = createRecommendationReason(deal);
  ...
  <section class="game-context"><div><p class="section-kicker">Why it made the cut</p><h2>Is ${escapeHTML(deal.title)} worth buying at this price?</h2><p>${escapeHTML(selectionReason)}</p><p>These permanent pages keep out add-ons, bundles, Early Access listings, and anything running on a thin review signal.</p></div><aside><h2>Before you buy ${escapeHTML(deal.title)}</h2><ul><li>${escapeHTML(storeCaveat(deal.storeName))}</li><li>The final price and the exact edition, on the store page.</li><li>Whether you will actually play it. The discount cannot answer that one.</li></ul></aside></section>
  ...
  <nav class="game-related" aria-label="Related pages"><a href="index.html">Browse more game price checks</a><a href="../deals/index.html">Browse live deal lists</a><a href="../methodology.html">See how Deal Scores work</a><a href="../blog.html">See weekly picks</a></nav></main>${footer()}</body></html>`;
}
