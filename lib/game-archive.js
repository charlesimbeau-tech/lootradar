...
function mergeArchive(previous, liveDeals, options = {}) {
  ...
  for (const [key, stored] of Object.entries(archive.games)) {
    ...
    // Recommendation text is derived from the carried evidence at render time.
    // Remove copies written by older archive versions as entries cycle through.
    delete games[key].recommendation;
  }
