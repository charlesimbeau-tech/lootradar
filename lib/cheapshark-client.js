(function attachCheapSharkClient(root, factory) {
  ...
  function createCheapSharkClient(options = {}) {
    ...
    const minRequestIntervalMs = Math.max(0, Number(options.minRequestIntervalMs || 0));
    ...
    function waitForRequestSlot() {
      if (minRequestIntervalMs <= 0) return Promise.resolve();

      // Serialize request starts, not just callers. This also covers retries and
      // concurrent lookups, which ad-hoc sleeps around client.get() cannot see.
