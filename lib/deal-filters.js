(function (root, factory) {
  ...
  function effectiveSort(input = {}) {
    const filters = normalizeFilters(input);
    // New arrivals is already filtered to the last year, so recency is the
    // entry condition rather than the ranking. Sorting by release date on top
    // of that let a day's head start bury a better-reviewed game: a 98%
    // positive release sat below an 88% one purely for being newer, which is
    // backwards on a site that ranks quality first everywhere else. The
    // collection's own floor of 80% across 500 reviews keeps thin ratings out.
    if (filters.collection === 'fresh') return 'reviewed';
