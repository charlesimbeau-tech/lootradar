(function (root, factory) {
  ...
  // The homepage opens on Best right now: a reader who arrives without asking
  // for anything should see the strongest deals first, not an alphabetical walk
  // through the whole qualified catalog. "All deals" is still one tab away.
  ...
  function effectiveSort(input = {}) {
    const filters = normalizeFilters(input);
    // Every named collection ranks by Deal Score. New arrivals used to be the
    // exception, sorted by release date, which let a day's head start bury a
    // better game: an 88% positive release sat above a 98% one. Recency is
    // already the entry condition for that collection, so sorting by it again
    // just repeated the filter. It also disagreed with the permanent
    // /deals/new-game-deals.html page, which has always ranked by Deal Score.
