  ...
  function createRecommendationReason(deal, result = calculateDealScore(deal)) {
    if (deal.userRating && deal.reviewCount) {
      ...
      if (result.usesHistoricalLow && deal.historicalLow) {
        ...
        if (difference <= 0.01) return `${review}, and this matches the lowest price we have on record.`;
        return `${review}, sitting $${difference.toFixed(2)} above its recorded low.`;
      }
      if (deal.discount >= 85) return `${review}, and it is a frankly silly ${deal.discount}% off.`;
      if (deal.discount >= 75) return `${review}, with a hefty ${deal.discount}% off on top.`;
      if (deal.discount >= 60) return `${review}, and ${deal.discount}% off is a proper cut.`;
      return `${review}. The discount is modest, so the game itself is doing the work here.`;
    }
    if (deal.criticScore) return `Critics gave it ${deal.criticScore}, but there are not enough player reviews to be sure.`;
    return 'The price looks sharp, though there is not much quality data to back it up.';
  }
