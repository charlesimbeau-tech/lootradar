(function (root, factory) {
  ...
  function createAccountClient(options) {
    ...
    function chooseGuestLocal(localProfile, localWatchlist) {
      ...
      if (owner && owner !== 'guest') {
        return {
          profile: data.normalizeProfile({}, timestamp()),
          watchlist: {}
        };
      }
      return {
    ...
    function saveUserCache(userId, profile, watchlist) {
      ...
      const writes = [
        storageSet(userCacheKey(PROFILE_STORAGE_KEY, userId), serializedProfile),
        storageSet(userCacheKey(WATCHLIST_STORAGE_KEY, userId), serializedWatchlist),
      ...
      return storageSet(CACHE_OWNER_KEY, userId);
    }
    ...
    function prepareWatchlist(watchlist, userId) {
      ...
      const previous = data.normalizeWatchlist(readUserCache(userId).watchlist, editTimestamp);
    ...
    function debouncedWrite(resource, input, prepare, writer) {
      return new Promise(function (resolve) {
        sessionContext().then(async function (context) {
          ...
          let locallySaved;
          if (!owner || owner === 'guest') {
            const guestProfile = parseStored(PROFILE_STORAGE_KEY, {});
            const guestWatchlist = parseStored(WATCHLIST_STORAGE_KEY, {});
            locallySaved = saveUserCache(
              context.user.id,
              resource === 'watchlist' ? guestProfile : value,
              resource === 'watchlist' ? value : guestWatchlist
            );
          } else {
