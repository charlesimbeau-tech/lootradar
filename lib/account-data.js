(function (root, factory) {
  ...
  function normalizeWatchItem(recordKey, value, now) {
    ...
    return { key, title, targetPrice, addedAt, updatedAt };
  }
  ...
  function mergeWatchlists(local, remoteRows) {
    const mergeTimestamp = new Date().toISOString();
    const localItems = normalizeWatchlist(local, mergeTimestamp);
    const remoteItems = normalizeWatchlist(remoteRows, mergeTimestamp);
    const merged = {};

    Object.keys(localItems).forEach(function (key) {
      merged[key] = cloneValue(localItems[key]);
    });
    Object.keys(remoteItems).forEach(function (key) {
      const localItem = merged[key];
      const remoteItem = remoteItems[key];
      if (!localItem || Date.parse(remoteItem.updatedAt) > Date.parse(localItem.updatedAt)) {
