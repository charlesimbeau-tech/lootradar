(function (root, factory) {
  ...
  function normalizeDeal(raw, stores = {}) {
    ...
    return {
      ...
      // Other stores carrying the same game, cheapest first. This is what makes
      // a game page an actual price comparison rather than a single quote.
      alternates: (Array.isArray(raw.alternates) ? raw.alternates : [])
        .map(offer => {
          const altSale = number(offer.salePrice, 0);
          const altNormal = number(offer.normalPrice, altSale);
          return {
            storeID: cleanText(offer.storeID),
            storeName: stores[offer.storeID]?.name || cleanText(offer.storeName) || 'Store',
            dealID: cleanText(offer.dealID),
            salePrice: altSale,
            normalPrice: altNormal,
            discount: number(
              offer.savings,
              altNormal > 0 ? ((altNormal - altSale) / altNormal) * 100 : 0
            )
          };
        })
        .filter(offer => offer.dealID && offer.salePrice >= 0)
